...
export const VGT_THEME_OPTIONS = [
...
export function normalizeThemeMode(mode) {
  if (typeof mode === "string") {
    ...
    if (normalized === "current" && typeof document !== "undefined") {
      return normalizeThemeMode(document.documentElement.getAttribute("data-theme"));
    }
  }
...
export function defaultConditionalFormatColors(themeMode) {
...
export function setAppTheme(mode, root = null) {
  const theme = normalizeThemeMode(mode);
  const target = root || (typeof document !== "undefined" ? document.documentElement : null);
  if (target) target.setAttribute("data-theme", theme);
  return theme;
}
